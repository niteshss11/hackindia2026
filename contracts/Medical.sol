...
contract Healthcare {
    ...
    mapping(uint => CureRecord) public cureRecords;
    mapping(uint => CureClaim) public cureClaims;
    mapping(uint => mapping(address => bool)) public patientDoctorAccess;
    ...
    uint public cureRecordCount;
    uint public cureClaimCount;
   ...
   //END PATIENTS
    ...
    event PatientRegistered(uint indexed patientId, address indexed account, string metadataURI);
    event DoctorRegistered(uint indexed doctorId, address indexed account, string metadataURI);
    event RecordUploaded(uint indexed recordId, uint indexed patientId, string recordType, string metadataURI);
    event AccessGranted(uint indexed patientId, address indexed doctor, string detail);
    event AccessRevoked(uint indexed patientId, address indexed doctor, string detail);
    event PrescriptionAdded(uint indexed patientId, address indexed doctor, string metadataURI);
    event ClaimCreated(uint indexed claimId, uint indexed patientId, string metadataURI);
    event ClaimApproved(uint indexed claimId, string detail);
    event ClaimRejected(uint indexed claimId, string detail);
    event EmergencyAccessRequested(uint indexed patientId, address indexed doctor, string detail);
    event AuditLogged(uint indexed auditId, address indexed actor, string action, string detail);
    ...
    function _logAudit(string memory _action, string memory _detail) internal {
        auditLogCount++;
        auditLogs[auditLogCount] = AuditLog(auditLogCount, msg.sender, _action, _detail, block.timestamp);
        emit AuditLogged(auditLogCount, msg.sender, _action, _detail);
    }
   ...
   function UPDATE_MEDICINE_ACTIVE(uint _medicineId) public onlyAdmin {
    ...
    //-------------- CURELEDGER TRACK 4 EXTENSIONS ------------------

    function registerPatient(string memory _metadataURI) public {
        require(!registeredPatients[msg.sender], "Patient already registered");
        string[] memory history = new string[](1);
        history[0] = "CureLedger registration";
        uint[] memory bought = new uint[](0);

        patientCount++;
        patients[patientCount] = Patient(patientCount, _metadataURI, history, msg.sender, bought);
        registeredPatients[msg.sender] = true;

        emit PatientRegistered(patientCount, msg.sender, _metadataURI);
        _logAudit("PatientRegistered", _metadataURI);
    }

    function registerDoctor(string memory _metadataURI) public {
        require(!registeredDoctors[msg.sender], "Doctor already registered");

        doctorCount++;
        doctors[doctorCount] = Doctor(doctorCount, _metadataURI, msg.sender, 0, 0, true);
        registeredDoctors[msg.sender] = true;

        emit DoctorRegistered(doctorCount, msg.sender, _metadataURI);
        _logAudit("DoctorRegistered", _metadataURI);
    }

    function uploadRecord(uint _patientId, string memory _metadataURI, string memory _recordType) public {
        require(_patientId <= patientCount, "Patient does not exist");
        require(
            patients[_patientId].accountAddress == msg.sender || registeredDoctors[msg.sender] || msg.sender == admin,
            "Not allowed to upload"
        );

        cureRecordCount++;
        cureRecords[cureRecordCount] = CureRecord(
            cureRecordCount,
            _patientId,
            _metadataURI,
            _recordType,
        ...
        patients[_patientId].medicalHistory.push(_metadataURI);

        emit RecordUploaded(cureRecordCount, _patientId, _recordType, _metadataURI);
        _logAudit("RecordUploaded", _metadataURI);
    }

    function uploadRecordHash(uint _patientId, string memory _recordHash, string memory _recordType) public {
        uploadRecord(_patientId, _recordHash, _recordType);
    }

    function grantAccess(uint _patientId, address _doctor, string memory _detail) public {
        require(_patientId <= patientCount, "Patient does not exist");
        require(patients[_patientId].accountAddress == msg.sender || msg.sender == admin, "Only patient/admin");

        patientDoctorAccess[_patientId][_doctor] = true;
        emit AccessGranted(_patientId, _doctor, _detail);
        _logAudit("AccessGranted", _detail);
    }

    function revokeAccess(uint _patientId, address _doctor, string memory _detail) public {
        require(_patientId <= patientCount, "Patient does not exist");
        require(patients[_patientId].accountAddress == msg.sender || msg.sender == admin, "Only patient/admin");

        patientDoctorAccess[_patientId][_doctor] = false;
        emit AccessRevoked(_patientId, _doctor, _detail);
        _logAudit("AccessRevoked", _detail);
    }

    function addPrescription(uint _patientId, string memory _metadataURI) public onlyDoctor {
        require(_patientId <= patientCount, "Patient does not exist");
        require(patientDoctorAccess[_patientId][msg.sender] || msg.sender == admin, "Access not granted");

        patients[_patientId].medicalHistory.push(_metadataURI);
        emit PrescriptionAdded(_patientId, msg.sender, _metadataURI);
        _logAudit("PrescriptionAdded", _metadataURI);
    }

    function createClaim(uint _patientId, string memory _metadataURI) public {
        require(_patientId <= patientCount, "Patient does not exist");
        require(patients[_patientId].accountAddress == msg.sender || msg.sender == admin, "Only patient/admin");

        cureClaimCount++;
        cureClaims[cureClaimCount] = CureClaim(cureClaimCount, _patientId, _metadataURI, "Pending", block.timestamp);

        emit ClaimCreated(cureClaimCount, _patientId, _metadataURI);
        _logAudit("ClaimCreated", _metadataURI);
    }

    function approveClaim(uint _claimId, string memory _detail) public onlyAdmin {
        require(_claimId <= cureClaimCount, "Claim does not exist");
        cureClaims[_claimId].status = "Approved";

        emit ClaimApproved(_claimId, _detail);
        _logAudit("ClaimApproved", _detail);
    }

    function rejectClaim(uint _claimId, string memory _detail) public onlyAdmin {
        require(_claimId <= cureClaimCount, "Claim does not exist");
        cureClaims[_claimId].status = "Rejected";

        emit ClaimRejected(_claimId, _detail);
        _logAudit("ClaimRejected", _detail);
    }

    function requestEmergencyAccess(uint _patientId, string memory _detail) public {
        require(_patientId <= patientCount, "Patient does not exist");
        require(registeredDoctors[msg.sender] || msg.sender == admin, "Only doctor/admin");

        emit EmergencyAccessRequested(_patientId, msg.sender, _detail);
        _logAudit("EmergencyAccessRequested", _detail);
    }

    function getAuditLogs() public view returns (AuditLog[] memory) {
        AuditLog[] memory logs = new AuditLog[](auditLogCount);
        for (uint i = 1; i <= auditLogCount; i++) {
            logs[i - 1] = auditLogs[i];
        }
        return logs;
    }

    function logAudit(string memory _action, string memory _detail) public {
        _logAudit(_action, _detail);
    }

    //-------------- END CURELEDGER TRACK 4 EXTENSIONS ------------------
