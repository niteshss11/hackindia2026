...
const changeNetwork = async ({ networkName }) => {
  try {
    ...
    if (!networks[networkName]) throw new Error(`Unsupported network: ${networkName || 'missing'}`)
    try {
...
export const PARSED_ERROR_MSG = (e) => {
  ...
  return json?.reason || json?.error?.message || e?.message || 'Something went wrong'
}
...
const GET_METADATA = async (url) => {
  ...
  if (url.startsWith('data:application/json,')) {
    return {
      data: JSON.parse(decodeURIComponent(url.replace('data:application/json,', ''))),
    }
  }
...
export const HEALTH_CARE_CONTARCT = async () => {
  ...
  if (!HEALTH_CARE_ADDRESS || !ethers.utils.isAddress(HEALTH_CARE_ADDRESS)) {
    throw new Error('Valid NEXT_PUBLIC_HEALTH_CARE contract address is required')
  }
  await HANDLE_NETWORK_SWITCH()
  const web3Modal = new Web3Modal()
  const connection = await web3Modal.connect()
  const provider = new ethers.providers.Web3Provider(connection)
  const contractCode = await provider.getCode(HEALTH_CARE_ADDRESS)
  if (contractCode === '0x') {
    throw new Error(
      `Healthcare contract not found on ${NETWORK || 'selected'} network. Check MetaMask network and NEXT_PUBLIC_HEALTH_CARE.`
    )
  }
...
export const CHECKI_IF_CONNECTED = async () => {
  if (typeof window === 'undefined') return 'Open this page in a browser'
...
export const GET_DOCTOR_DETAILS = async (_doctorId) => {
  ...
  } = await GET_METADATA(doctor.IPFS_URL)
...
export const GET_MEDICINE_DETAILS = async (_medicineId) => {
  ...
  } = await GET_METADATA(medic.IPFS_URL)
...
export const GET_PATIENT_DETAILS = async (_patientId) => {
  ...
  } = await GET_METADATA(patient.IPFS_URL)
...
export const UPLOAD_IPFS_IMAGE = async (file) => {
  if (file) {
    try {
      if (!PINATA_AIP_KEY || !PINATA_SECRECT_KEY) {
...
export const UPLOAD_METADATA = async (data) => {
  try {
    ...
    return `data:application/json,${encodeURIComponent(normalizedData)}`
  }
}
...
export const ASK_AI_CHAT = async (prompt) => {
  ...
  if (!apiKey) {
    return 'OpenAI API key is missing. Add NEXT_PUBLIC_OPEN_AI_KEY to your environment.'
  }
